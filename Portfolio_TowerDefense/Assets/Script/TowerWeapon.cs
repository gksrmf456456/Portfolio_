...
public class TowerWeapon : MonoBehaviour
{
    [Header("Commons")]
    [SerializeField]
    private TowerTemplate towerTemplate;                            // 타워정보 (공격력, 공격속도 등)
    [SerializeField]                                             
    ...
    private PlayerGold playerGold;                                  // 플레이어 골드 정보 획득 및 설정
    private Tile ownerTile;                                         // 현재 타워가 배치되어 있는 타일

    private float addedDamage;                                      // 버프에 의해 추가된 데미지
    private int buffLevel;                                          // 버프를 받는지 여부 설정 (0 : 버프 x, 1~3 : 버프 받는 레벨)
    ...
    public void Setup(TowerSpawner towerSpawner, EnemySpawner enemySpawner, PlayerGold playerGold, Tile ownerTile)
    {
        ...
        {
            // 최초 상태를 WeaponState.SearchTarget 로 지정
    ...
    private void RotateToTarget()
    {
        ...
        // 각도가 radian 단위기 때문에 Mathf.Rad2Deg 를 곱해 도 단위를 구함
    ...
    {
        // target이 있는지 검사 (다른 발사체에의해 제거, Goal 지점까지 이동해 삭제 등)
        if (attackTarget == null)
        {
            return false;
        }

        // target 이 공격범위 안에 있는지 검사(공격범위를 벗어나면 새로운 적 탐색)
    ...
    IEnumerator TryAttackCannon()
    {
        while (true)
        {
            ...
            //// 2. target 이 공격 범위 안에 있는지 검사 (공격범위를 벗어나면 새로운 적 탐색)
            //float distance = Vector3.Distance(attackTarget.position, transform.position);
            //if (distance > towerTemplate.weapon[level].range)
            //{
            //    attackTarget = null;
    ...
    private void SpawnProjectile()
    {
        ...
        // 생성된 발사체에게 공겨대상 (attackTarget) 정보 제공
    ...
    private void SpawnLaser()
    {
        ...
        // 같은 방향으로 여러개의 관선을 쏴서 그 중 현재 attackTarget과 동일한 오브젝트를
    ...
    public bool Upgrade()
    {
        ...
        // 현재 타워가 버프 타워인 경우, 현재 타워가 공격타워인 경우
